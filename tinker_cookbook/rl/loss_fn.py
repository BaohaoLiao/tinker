...
def compute_ppo_loss(
    ...
    assert clip_ratio_c > 1.0, (
        f"clip_ratio_c must be greater than 1.0, got {clip_ratio_c}"
    )
    ...
    pg_losses2 = -all_advantages * torch.clamp(
    ...
    with torch.no_grad():
        ...
        clip_fraction = (
            (torch.gt(pg_losses2, pg_losses1).float() * all_masks).sum() 
            / all_masks.sum().clamp(min=1.0)
        )
        
        # Lower clip fraction: how often lower bound was active (for negative advantages)
        lower_clip_fraction = (
            (torch.gt(clip_pg_losses1, pg_losses3).float() * (all_advantages < 0).float() * all_masks).sum()
            / all_masks.sum().clamp(min=1.0)
        )
